...
class LitOCRXL(pl.LightningModule):
    ...
    def setup(self, stage=None):
        self.decoder.set_device(
            self.device
        )  # to set the device for the decoder tensors created in the forward pass
    ...
    def teacher(self, imgs, transcriptions, train=False):
        ...
        dec_input = torch.full(
            (batch_size, 1), pad_id, device=self.device, dtype=torch.long
        )
        ...
        for i in range(1, transcriptions.size(1)):
            ...
            dec_output, mem = self.decoder(
                dec_input.detach(), enc_output.detach(), mem=mem, out_idx=out_idx
            )
    ...
    def greedy_search(self, imgs):
        ...
        while not is_all_eos and outputs.size(1) < self.max_len:
            # Forward pass
            out_idx = (
                outputs.size(1) - 1 if outputs.size(1) < self.context_length else -1
            )
            dec_output, mem = self.decoder(
                dec_input, enc_output, mem=mem, out_idx=out_idx
            )
            dec_out_max = dec_output.argmax(-1)
            out_eos_pad = torch.logical_or(
                outputs[:, -1] == eos_id, outputs[:, -1] == pad_id
            )
            ...
            # Checking is_all_eos
            is_all_eos = torch.all(
                torch.logical_or(outputs[:, -1] == eos_id, outputs[:, -1] == pad_id)
            )
        return outputs
