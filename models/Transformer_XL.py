...
class PositionalEmbedding(nn.Module):
    def __init__(self, d, **kwargs):
        ...
        self.d = d
        inv_freq = 1 / (
            10000 ** (torch.arange(0.0, d, 2.0) / d)
        )  # Calculate the inverse frequency to be used in the positional encoding
        self.register_buffer("inv_freq", inv_freq)

    def forward(self, pos):
        sinusoid_inp = torch.einsum(
            "i,j->ij", pos, self.inv_freq
        )  # multiply each position i in the pos array with each position j in the inv_freq array
...
class RecurrenceAttention(nn.Module):
    def __init__(
        self, d_model, d_inner, num_heads, dropout=0.1, pre_norm=False, **kwargs
    ):
        super(RecurrenceAttention, self).__init__(**kwargs)
        self.d_model = d_model
        self.d_inner = (
            d_inner  # the inner dimension of the query, key and value matrices
        )
        self.num_heads = num_heads
        self.d_head = d_inner // num_heads

        self.q = nn.Linear(
            d_model, d_inner, bias=False
        )  # to project the query to the inner dimension
        self.kv = nn.Linear(
            d_model, d_inner * 2, bias=False
        )  # to project the key and value to the inner dimension

        self.drop = nn.Dropout(dropout)
        self.fc = nn.Linear(
            self.d_inner, d_model
        )  # to project the output of the multi-head attention to the original dimension
    ...
    def forward(self, x, pos_emb, u, v, tgt_mask=None, pad_mask=None, mem=None):
        ...
        q_tfmd, kv = self.q(x), self.kv(
            h_telda
        )  # Calculate the query and key-value matrices
        k_tfmd, v_tfmd = torch.chunk(
            kv, 2, dim=-1
        )  # Split the key-value matrix into key and value matrices # (b, seq_len + prev_seq, d_inner * num_heads)
        q_tfmd, k_tfmd, v_tfmd = (
            q_tfmd.view(batch_size, cur_seq, self.num_heads, self.d_head),
            k_tfmd.view(batch_size, cur_seq + prev_seq, self.num_heads, self.d_head),
            v_tfmd.view(batch_size, cur_seq + prev_seq, self.num_heads, self.d_head),
        )

        # Calculate the attention
        content_attn = torch.einsum(
            "bihd,bjhd->bijh", q_tfmd + u, k_tfmd
        )  # Calculate the content-based attention
        pos_emb = pos_emb.view(cur_seq + prev_seq, self.num_heads, self.d_head)
        pos_attn = torch.einsum(
            "bihd,jhd->bijh", q_tfmd + v, pos_emb
        )  # Calculate the position-based attention
        position_attn = self._rel_shift(pos_attn)  # Shift the position-based attention
        attn = (
            content_attn + position_attn
        )  # Combine the content-based and position-based attention

        if (
            tgt_mask is not None
        ):  # Apply the mask to the attention shape: (batch_size, cur_seq, cur_seq + prev_seq, num_heads)
            attn = attn.masked_fill(
                tgt_mask, float("-inf")
            )  # Apply the mask to the attention
        ...
        attn = self.drop(attn)
        weightd_val = (
            torch.einsum("bijh,bjhd->bihd", attn, v_tfmd)
            .contiguous()
            .view(batch_size, cur_seq, -1)
        )
    ...
    def _scaled_dot_product_attention(self, q, k, v, mask, padding_mask):
        ...
        if mask is not None:
            qk = qk.masked_fill(
                mask == 0, float("-inf")
            )  # Apply the mask to the scaled dot product attention
...
class CrossAttention(nn.Module):
    ...
    def forward(self, dec, enc):
        ...
        output = (
            torch.einsum("bijh,bjhd->bihd", attn, v)
            .contiguous()
            .view(batch, dec_len, -1)
        )
...
class TransformerDecoderLayerRelative(nn.Module):
    ...
    def forward(
        self, x, enc_output, pos_emb, u, v, mem=None, tgt_mask=None, pad_mask=None
    ):
        if not self.pre_norm:
            out = self.multi_head_attention(
                x, pos_emb, u, v, tgt_mask=tgt_mask, pad_mask=pad_mask, mem=mem
            )
            ...
            out = self.multi_head_attention(
                out, pos_emb, u, v, tgt_mask=tgt_mask, pad_mask=pad_mask, mem=mem
            )
            out = x + out
...
class TransformerXL(nn.Module):
    def __init__(
        ...
        **kwargs
        ...
        for _ in range(n_layers):
            self.layers.append(
                TransformerDecoderLayerRelative(
                    d_model, n_heads, dff, d_inner, dropout=dropout
                )
            )
    ...
    def forward(
        self, x, enc_output, mem=None, out_idx=-1
    ):  # out_idx is the index of the output layer to return, because of padding
        ...
        tgt_mask = (
            torch.triu(
                torch.ones(seq_len, seq_len + prev_seq)[..., None],
                diagonal=1 + prev_seq,
            )
            .bool()
            .to(self.device)
        ...
        pos_idxs = torch.arange(seq_len + prev_seq - 1, -1, -1.0, dtype=torch.float).to(
            self.device
        )
